...
function apply(ctx, config) {
  ...
  const buildTextLinkResponseData = /* @__PURE__ */ __name((song) => {
    if (!song || typeof song !== "object") return null;
    const name2 = String(song.name ?? song.title ?? "").trim();
    const artist = resolveTextLinkArtist(song);
    const musicUrl = String(song.url ?? song.musicUrl ?? "").trim();
  ...
  ctx.on("ready", async () => {
    ...
    if (config.enableMiddleware) {
      ctx.middleware(async (session, next) => {
        ...
        if (!String(config.apilink || "").trim()) {
          logger.warn("文本音乐链接解析跳过：apilink 未配置。");
          return next();
        }
        let metingSong = null;
        try {
          metingSong = await fetchMetingSongDetailById(parsedTextLink.songId);
        } catch (error) {
          if (isMetingAbnormalResponseError(error)) {
            logger.warn("文本音乐链接详情响应异常:", error?.message || error);
            return next();
          }
          logger.error("文本音乐链接详情获取失败:", error);
          return next();
        }
        if (!metingSong) {
          logger.info("文本音乐链接解析无结果：未获取到歌曲详情。", parsedTextLink);
          return next();
        }
        const responseData = buildTextLinkResponseData(metingSong);
        if (!responseData) {
          logger.warn("文本音乐链接解析结果缺少可用 musicUrl，已跳过发送。", {
            songId: parsedTextLink.songId,
