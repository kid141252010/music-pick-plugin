...
function apply(ctx, config) {
  ...
  async function downloadMediaBuffer(url, tag) {
    const headerStrategies = [
      ...
      // 策略2：模拟移动端 QQ 音乐请求
      {
        "User-Agent": "Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36 QQMusic/13.0.0",
      ...
      // 策略3：模拟 QQ 音乐桌面客户端
      {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.164 Safari/537.36 QQBrowser/11.9.5325.400",
        Referer: "https://y.qq.com/",
        Accept: "*/*",
        "Accept-Encoding": "identity;q=1, *;q=0"
      },
      // 策略4：纯净请求，不带 Referer/Origin
      {
        ...customHeaders,
        Accept: "*/*",
        "Accept-Encoding": "identity;q=1, *;q=0"
    ...
    for (let i = 0; i < headerStrategies.length; i++) {
      const headers = headerStrategies[i];
      try {
        logger.info(`[${tag}] 下载策略 #${i + 1}/${headerStrategies.length}`);
        const options = createMediaDownloadOptions(headers);
        // 允许跟随重定向，并设置 maxRedirects
        options.maxRedirects = 5;
        const arrayBuffer = await retryableGet(url, options, `${tag} #${i + 1}`);
        const buf = Buffer.from(arrayBuffer);
        if (!buf.length) {
          logger.warn(`[${tag}] 策略 #${i + 1} 返回空 body，跳过。`);
          lastError = new Error("empty download body");
          continue;
        }
        logger.info(`[${tag}] 策略 #${i + 1} 成功，大小: ${buf.length} 字节`);
        return buf;
      } catch (error) {
        lastError = error;
        const status = error?.response?.status;
        logger.warn(`[${tag}] 策略 #${i + 1} 失败(status=${status ?? "unknown"}): ${error?.message || error}`);
      }
    }
    // 如果所有策略都失败，尝试通过 API 代理下载
    try {
      logger.info(`[${tag}] 所有直连策略失败，尝试通过 API 代理下载`);
      const proxyDownloadUrl = `https://${config.apilink}/api/web/proxy/download?url=${encodeURIComponent(url)}`;
      const options = createMediaDownloadOptions(customHeaders);
      options.maxRedirects = 5;
      const arrayBuffer = await retryableGet(proxyDownloadUrl, options, `${tag} proxy`);
      const buf = Buffer.from(arrayBuffer);
      if (buf.length) {
        logger.info(`[${tag}] API 代理下载成功，大小: ${buf.length} 字节`);
        return buf;
      }
    } catch (proxyError) {
      logger.warn(`[${tag}] API 代理下载也失败:`, proxyError?.message || proxyError);
    }
    throw lastError;
  }
...
`));
        ...
        case "image":
          ...
          break;
        case "audio": {
          // 优先下载后以 Buffer 发送，避免 OneBot 对远程 URL 触发 download_file 失败
          try {
            ...
            audioElements.push(import_koishi.h.audio(buf, mimeType));
          } catch (e) {
            const status = e?.response?.status;
            logger.warn(`音频下载失败(status=${status ?? "unknown"})，回退为直链发送:`, e?.message || e);
            // 回退1：尝试通过 base64 data URI 发送
            try {
              logger.info(`[Audio] 尝试使用简单 GET 下载并转 base64`);
              const resp = await ctx.http.get(value, createHttpOptions({
                responseType: "arraybuffer",
                headers: { "User-Agent": customHeaders["User-Agent"], Accept: "*/*" },
                timeout: 60000,
                maxRedirects: 10
              }));
              const fallbackBuf = Buffer.from(resp);
              if (fallbackBuf.length > 0) {
                let mimeType = "audio/mpeg";
                if (value.includes(".flac")) mimeType = "audio/flac";
                else if (value.includes(".ogg")) mimeType = "audio/ogg";
                else if (value.includes(".m4a") || value.includes(".aac")) mimeType = "audio/mp4";
                const base64Url = `data:${mimeType};base64,${fallbackBuf.toString("base64")}`;
                audioElements.push(import_koishi.h.audio(base64Url));
                logger.info(`[Audio] base64 回退成功，大小: ${fallbackBuf.length} 字节`);
              } else {
                throw new Error("empty response");
              }
            } catch (e2) {
              logger.warn(`[Audio] base64 回退也失败:`, e2?.message || e2);
              // 回退2：直接发送 URL，让 OneBot 或客户端自行下载
              audioElements.push(import_koishi.h.audio(value));
            }
          }
          break;
        }
        ...
        case "file": {
          try {
            ...
            audioElements.push((0, import_koishi.h)("file", { src: buf, title: filename, name: filename }));
          } catch (e) {
            const status = e?.response?.status;
