...
function apply(ctx, config) {
  ...
  const getSearchApiUrl = /* @__PURE__ */ __name((source, keyword) => {
    if (source === SOURCE_NETEASE) {
      const neteaseBase = normalizeApiBase(config.neteaseApiLink);
      if (!neteaseBase) return "";
      return `${neteaseBase}/cloudsearch?keywords=${encodeURIComponent(keyword)}&limit=${config.searchLimit}&type=1&offset=0&randomCNIP=true`;
    }
    const metBase = normalizeApiBase(config.apilink);
    if (!metBase) return "";
    return `${metBase}/api/web/cloudsearch?keywords=${encodeURIComponent(keyword)}&limit=${config.searchLimit}&type=1&offset=0`;
  }, "getSearchApiUrl");
  const getSongUrlApi = /* @__PURE__ */ __name((source, songId) => {
    if (source === SOURCE_NETEASE) {
      ...
      return `${neteaseBase}/song/url/v1?id=${songId}&level=${neteaseLevel}${osParam}&unblock=true&timestamp=${Date.now()}&randomCNIP=true`;
    }
    const metBase = normalizeApiBase(config.apilink);
    if (!metBase) return "";
  ...
  async function fetchSongsByKeyword(session, keyword, requesterTag, source, options = {}) {
    if (source !== SOURCE_NETEASE && !config.apilink) {
      return { error: `${requesterTag}API 链接未配置，请联系管理员在插件配置中设置 \`apilink\`。` };
    }
    if (source === SOURCE_NETEASE && !String(config.neteaseApiLink || "").trim()) {
      return { error: `${requesterTag}网易云源未配置，请联系管理员。` };
    }
    if (!keyword) {
      return { error: `${requesterTag}请输入要搜索的歌曲关键词。` };
    }
    let searchMsgIds = [];
    ...
    try {
      const searchUrl = getSearchApiUrl(source, keyword);
      if (!searchUrl) {
        return { error: `${requesterTag}搜索服务未配置，请联系管理员。` };
      }
      ...
      if (response.code !== 200 || !response.result?.songs?.length) {
        return { error: `${requesterTag}未能找到任何相关歌曲，请试试更换关键词。` };
      }
      return { songs: response.result.songs };
    } catch (error) {
      logger.error("Search API request failed:", error);
      return { error: `${requesterTag}歌曲列表获取失败，可能是网络问题或 API 暂时不可用。` };
    } finally {
  ...
  async function registerPendingSongSelection(session, requesterTag, keyword, songs, promptMsgIds, source) {
  ...
  async function beginInteractiveSongRequest(session, request) {
    const { keyword, requesterTag, source } = request;
    const pendingKey = getPendingSelectionKey(session);
    ...
    const searchResult = await fetchSongsByKeyword(session, keyword, requesterTag, source, { showSearchHint: true });
    ...
    await registerPendingSongSelection(session, requesterTag, keyword, songs, promptResult.promptMsgIds || [], source);
    return;
  }
  __name(beginInteractiveSongRequest, "beginInteractiveSongRequest");
  async function resolveSelectedSongResponse(session, requesterTag, selectedSong, source) {
    let musicUrl;
    try {
      if (source === SOURCE_NETEASE && !String(config.neteaseApiLink || "").trim()) {
  ...
  async function executeSongRequest(session, request) {
    ...
    if (!number) {
      return beginInteractiveSongRequest(session, { ...request, keyword, requesterTag, source });
    }
    const searchResult = await fetchSongsByKeyword(session, keyword, requesterTag, source);
    if (searchResult.error) return searchResult.error;
    const songs = searchResult.songs;
    if (number <= 0 || number > songs.length) {
      return `${requesterTag}序号无效。搜索结果共有 ${songs.length} 首歌曲。`;
    }
    return resolveSelectedSongResponse(session, requesterTag, songs[number - 1], source);
  }
  ...
  ctx.middleware(async (session, next) => {
    ...
    try {
      const result = await resolveSelectedSongResponse(session, removed.requesterTag, removed.songs[index - 1], removed.source || DEFAULT_SOURCE);
      if (result) {
  ...
  ctx.command(`${config.commandName} <keyword:text>`, "搜索和点播音乐。").option("number", "-n <value:posint>").option("netease163", "-163").option("wyy", "-wyy").action(async ({ session, options }, keyword) => {
    ...
    const source = rawMeta.forceNetease ? SOURCE_NETEASE : DEFAULT_SOURCE;
