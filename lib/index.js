...
function apply(ctx, config) {
  ...
  ctx.command(`${config.commandName} <keyword:text>`, "搜索和点播音乐。").option("number", "-n <value:posint>").action(async ({ session, options }, keyword) => {
    ...
    try {
      ...
      const response = await ctx.http.get(searchUrl, createHttpOptions({ headers: customHeaders }));
...
`));
        ...
        case "audio": {
          ...
          try {
            ...
            const arrayBuffer = await ctx.http.get(value, createHttpOptions({
            ...
            logger.warn("音频下载失败，回退为文本链接发送:", e?.message || e);
            elements.push(import_koishi.h.text(`${describe}: ${value}\n`));
          }
          break;
        }
        ...
        case "file": {
          try {
            ...
            logger.warn("文件下载失败，回退为文本链接发送:", e?.message || e);
            elements.push(import_koishi.h.text(`${describe}: ${value}\n`));
          }
          break;
        }
