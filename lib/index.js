...
function apply(ctx, config) {
  ...
  const getSearchApiUrl = /* @__PURE__ */ __name((source, keyword) => {
    if (source === SOURCE_NETEASE) {
      ...
      return `${neteaseBase}/cloudsearch?keywords=${encodeURIComponent(keyword)}&limit=${config.searchLimit}&type=1&offset=0`;
    }
  ...
  const getSongUrlApi = /* @__PURE__ */ __name((source, songId) => {
    if (source === SOURCE_NETEASE) {
      ...
      return `${neteaseBase}/song/url/v1?id=${songId}&level=${neteaseLevel}&unblock=true&timestamp=${Date.now()}`;
    }
    const metBase = normalizeApiBase(config.apilink);
    if (!metBase) return "";
