...
function apply(ctx, config) {
  ...
  const tempMediaDir = import_node_path.join((0, import_node_os.tmpdir)(), "koishi-met-music-link");
  async function createTempMediaFile(buffer, ext, tag) {
    const safeExt = String(ext || "bin").replace(/[^a-z0-9]/gi, "").toLowerCase() || "bin";
    const token = `${Date.now()}-${Math.random().toString(16).slice(2, 10)}`;
    const filePath = import_node_path.join(tempMediaDir, `${tag}-${token}.${safeExt}`);
    await import_node_fs.mkdir(tempMediaDir, { recursive: true });
    await import_node_fs.writeFile(filePath, buffer);
    return filePath;
  }
  __name(createTempMediaFile, "createTempMediaFile");
  async function cleanupTempFiles(filePaths) {
    for (const filePath of filePaths) {
      try {
        await import_node_fs.unlink(filePath);
      } catch {
      }
    }
  }
  ...
  async function _downloadMediaBuffer(url, tag) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = mod.get(reqOptions, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          res.resume();
          _downloadMediaBuffer(res.headers.location, tag).then(resolve).catch(reject);
          return;
        }
        ...
        res.on("end", () => {
          ...
          if (!buf.length) { reject(new Error("empty download body")); return; }
  ...
  async function downloadMediaBuffer(url, tag) {
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
      try {
        return await _downloadMediaBuffer(url, tag);
      } catch (error) {
        lastError = error;
        if (attempt >= retryCount || !shouldRetryError(error)) throw error;
...
`));
        ...
        case "audio": {
          ...
          try {
            ...
            // 绕过 Koishi OneBot 适配器的拦截（防止其触发 download_file API 导致超时），直接让底层 NapCat 处理 base64
            const base64Url = `base64://${buf.toString("base64")}`;
        ...
        case "file": {
          ...
          try {
            ...
            if (oneBotLike) {
              // onebot 的 file 元素会调用 download_file，base64 URL 容易超时；改为本地临时文件上传
              const tempPath = await createTempMediaFile(buf, ext, "music-file");
              tempFilePaths.push(tempPath);
              const fileUrl = (0, import_node_url.pathToFileURL)(tempPath).href;
            ...
            logger.warn(`文件下载失败(status=${status ?? "unknown"}):`, e?.response?.data || e?.message || e);
            elements.push(import_koishi.h.text(`${describe}: 文件下载失败（${status ? `HTTP ${status}` : e?.message || "未知错误"}），请检查代理配置是否正确。\n`));
          }
          break;
        }
      }
