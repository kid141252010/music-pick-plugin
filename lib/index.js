...
function apply(ctx, config) {
  if (ctx.server) {
    ctx.server.get("/music-pick/:id", (koaCtx) => {
  ...
  const resolveServerDownloadBaseUrl = /* @__PURE__ */ __name((requirePublic = false) => {
  ...
  const createCachedMediaUrl = /* @__PURE__ */ __name((buf, mimeType = "application/octet-stream", requirePublic = false) => {
    if (!ctx.server) return "";
    const baseUrl = resolveServerDownloadBaseUrl(requirePublic);
    if (!baseUrl) return "";
    const id = Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
    fileCache.set(id, { buf, mimeType });
    setTimeout(() => fileCache.delete(id), 5 * 60 * 1000);
    return `${baseUrl}/music-pick/${id}`;
  }, "createCachedMediaUrl");
  const selectRemoteUploadUrl = /* @__PURE__ */ __name((sourceUrl, buf, mimeType, tag) => {
    const cachedUrl = createCachedMediaUrl(buf, mimeType, true);
    if (cachedUrl) {
      if (isPublicHttpUrl(sourceUrl)) {
        logger.info(`[${tag}] 大文件远程上传优先使用插件缓存链接，已跳过上游直链。缓存地址: ${cachedUrl}`);
      } else {
        logger.info(`[${tag}] 上游直链不可用于远程上传，使用插件缓存链接: ${cachedUrl}`);
      }
      return cachedUrl;
    }
    const sourcePublic = isPublicHttpUrl(sourceUrl);
    if (sourcePublic) {
      logger.info(`[${tag}] 未配置可公网访问的 selfUrl，回退使用上游直链: ${sourceUrl}`);
      return sourceUrl;
    }
    logger.info(`[${tag}] 未找到可用远程上传链接：缓存地址不可用且上游直链不可公网访问。`);
    return "";
  ...
  const songRequestQueues = /* @__PURE__ */ new Map();
  let requestSeed = 0;
  function getSessionQueueKey(session) {
    return session.fid || `${session.platform}:${session.channelId}:${session.userId}`;
  }
  __name(getSessionQueueKey, "getSessionQueueKey");
  ...
  function enqueueSongRequest(session, payload, options = {}) {
    const queueKey = getSessionQueueKey(session);
    let state = songRequestQueues.get(queueKey);
    if (!state) {
      state = { running: false, tasks: [] };
      songRequestQueues.set(queueKey, state);
    }
    const aheadCount = (state.running ? 1 : 0) + state.tasks.length;
    ...
    const task = {
      ...
      requesterTag: getRequesterTag(session),
      autoSend: options.forceAutoSend ?? aheadCount > 0,
      deferred
    };
    state.tasks.push(task);
    logInfo(`[Queue] ${task.requesterTag} 入队，key=${queueKey}，前方=${aheadCount}，当前队列=${state.tasks.length}`);
    void processSongQueue(queueKey);
    const queuedMessage = aheadCount > 0 ? `${task.requesterTag}已加入点歌队列，前方还有 ${aheadCount} 个请求。` : "";
    return { queuedMessage, deferred };
  }
  __name(enqueueSongRequest, "enqueueSongRequest");
  async function processSongQueue(queueKey) {
    const state = songRequestQueues.get(queueKey);
    ...
    try {
      while (state.tasks.length) {
        ...
        if (!task) continue;
        try {
          const result = await executeSongRequest(task.session, task);
          if (task.autoSend && result) {
            await retryableSend(task.session, result, "点歌结果");
          }
          task.deferred.resolve(result);
        } catch (error) {
          logger.error("[Queue] 点歌任务执行失败:", error);
          const fallback = `${task.requesterTag}点歌处理失败，请稍后重试。`;
          if (task.autoSend) {
            try {
              await retryableSend(task.session, fallback, "点歌队列错误提示");
            } catch (sendError) {
              logger.error("[Queue] 错误提示发送失败:", sendError);
            }
            task.deferred.resolve();
          } else {
            task.deferred.resolve(fallback);
          }
        }
      }
    } finally {
      ...
      if (!state.tasks.length) {
        songRequestQueues.delete(queueKey);
      } else {
        void processSongQueue(queueKey);
      }
    }
  }
  __name(processSongQueue, "processSongQueue");
  async function executeSongRequest(session, request) {
    const { keyword, number, requesterTag } = request;
    if (!config.apilink) {
      return `${requesterTag}API 链接未配置，请联系管理员在插件配置中设置 \`apilink\`。`;
    }
    ...
    try {
      ...
      if (response.code !== 200 || !response.result?.songs?.length) {
        if (searchMsgIds.length) await safeRecall(session, searchMsgIds);
        return `${requesterTag}未能找到任何相关歌曲，请试试更换关键词。`;
      }
      songs = response.result.songs;
    } catch (error) {
      logger.error("Search API request failed:", error);
      if (searchMsgIds.length) await safeRecall(session, searchMsgIds);
      return `${requesterTag}歌曲列表获取失败，可能是网络问题或 API 暂时不可用。`;
    }
    if (searchMsgIds.length) await safeRecall(session, searchMsgIds);
    let selectedSong;
    if (number) {
      if (number > 0 && number <= songs.length) {
        selectedSong = songs[number - 1];
      } else {
      ...
      const exitCommands = config.exitCommand.split(",").map((cmd) => cmd.trim()).filter(Boolean);
      const exitHint = exitCommands[0] || "退出";
      const prompt = `${requesterTag}请在 ${config.waitTimeout} 秒内输入序号选择歌曲，输入"${exitHint}"可退出。`;
      let promptMsgIds = [];
      if (config.imageMode) {
        if (!ctx.puppeteer) return `${requesterTag}\`puppeteer\` 服务未启用，无法生成图片列表。`;
        const image = await generateSongListImage(songList);
        if (!image) {
          return `${requesterTag}歌曲列表图片生成失败，请检查后台日志。`;
        }
        try {
          ...
          return `${requesterTag}发送歌曲选择列表失败，请稍后重试。`;
        }
      } else {
      ...
      if (promptMsgIds.length) await safeRecall(session, promptMsgIds);
      if (!userInput) return `${requesterTag}选择超时，已自动退出点歌。`;
      if (exitCommands.length && exitCommands.some((cmd) => userInput.trim() === cmd)) {
        return `${requesterTag}已退出点歌。`;
      }
      const index = parseInt(userInput, 10);
      if (isNaN(index) || index < 1 || index > songs.length) {
        return `${requesterTag}无效的序号，请重新点歌。`;
      }
          ...
          for (const element of elements) {
            if (element.type === "json" && element.attrs?.data) {
              ...
              if (musicMeta?.tag?.includes("音乐")) {
                ...
                const enqueueResult = enqueueSongRequest(session, {
                  keyword,
                  number: 1,
                  source: "middleware"
                }, { forceAutoSend: true });
                if (enqueueResult.queuedMessage) {
                  try {
                    await retryableSend(session, enqueueResult.queuedMessage, "点歌排队提示");
                  } catch (error) {
                    logger.warn("点歌排队提示发送失败:", error?.message || error);
                  }
                }
                return;
              }
  ...
  ctx.command(`${config.commandName} <keyword:text>`, "搜索和点播音乐。").option("number", "-n <value:posint>").action(async ({ session, options }, keyword) => {
    ...
    if (enqueueResult.queuedMessage) return enqueueResult.queuedMessage;
    return enqueueResult.deferred.promise;
  });
...
`));
        ...
        case "audio": {
          ...
          try {
            ...
            if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
              const remoteUrl = selectRemoteUploadUrl(value, buf, mimeType, "Audio");
              if (!remoteUrl) {
                const failMsg = `[Audio] 大文件(${formatSize(buf.length)})远程上传失败：请配置可公网访问的 selfUrl 或使用可直连音频源。`;
              ...
              fileUrl = createCachedMediaUrl(buf, mimeType, false);
              if (!fileUrl) {
                logger.warn(`[Audio] 文件超过内联阈值，但未配置可被 OneBot 访问的 selfUrl，降级为文本链接发送。`);
        ...
        case "file": {
          ...
          try {
            ...
            let fileUrl;
            if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
              const remoteUrl = selectRemoteUploadUrl(value, buf, "application/octet-stream", "File");
              if (!remoteUrl) {
                const failMsg = `[File] 大文件(${formatSize(buf.length)})远程上传失败：请配置可公网访问的 selfUrl 或使用可直连文件源。`;
                if (onebotStrictRemoteUpload) {
                  throw new Error(failMsg);
                }
                logger.warn(failMsg);
              ...
              fileUrl = createCachedMediaUrl(buf, "application/octet-stream", false);
              if (!fileUrl) {
                logger.warn(`[File] 文件超过内联阈值，但未配置可被 OneBot 访问的 selfUrl，降级为文本链接发送。`);
