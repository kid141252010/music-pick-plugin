...
function apply(ctx, config) {
  ...
  async function downloadMediaBuffer(url, tag) {
    const headerStrategies = [
      // 策略1：QQ 音乐标准 Referer + Origin
      { ...customHeaders, Referer: "https://y.qq.com/", Origin: "https://y.qq.com" },
      // 策略2：换 music.qq.com
      { ...customHeaders, Referer: "https://music.qq.com/", Origin: "https://music.qq.com" },
      // 策略3：不带 Referer/Origin，纯 UA
      { ...customHeaders },
      // 策略4：完全裸请求
    ...
    for (let i = 0; i < headerStrategies.length; i++) {
      ...
      try {
        ...
        const arrayBuffer = await retryableGet(url, createMediaDownloadOptions(headers), `${tag} #${i + 1}`);
...
`));
        ...
        case "audio": {
          ...
          try {
            ...
            audioElements.push(import_koishi.h.audio(buf, "audio/mpeg"));
          } catch (e) {
            const status = e?.response?.status;
            logger.warn(`音频下载失败(status=${status ?? "unknown"})，回退为直链发送:`, e?.message || e);
            // 回退：直接发送 URL，让 OneBot 或客户端自行下载
