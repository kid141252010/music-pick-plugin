...
function apply(ctx, config) {
  ...
  const selectRemoteUploadUrl = /* @__PURE__ */ __name((sourceUrl, buf, mimeType, tag) => {
    ...
    logger.info(`[${tag}] 未找到可用远程上传链接：请配置可公网访问的 selfUrl。`);
    return "";
        ...
        case "audio": {
          ...
          try {
            ...
            if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
              ...
              if (!remoteUrl) {
                const failMsg = `[Audio] 大文件(${formatSize(buf.length)})远程上传失败：请配置可公网访问的 selfUrl。`;
        ...
        case "file": {
          ...
          try {
            ...
            if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
              ...
              if (!remoteUrl) {
                const failMsg = `[File] 大文件(${formatSize(buf.length)})远程上传失败：请配置可公网访问的 selfUrl。`;
                if (onebotStrictRemoteUpload) {
                  throw new Error(failMsg);
                }
                logger.warn(failMsg);
