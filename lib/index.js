...
function apply(ctx, config) {
  ...
  ctx.command(`${config.commandName} <keyword:text>`, "搜索和点播音乐。").option("number", "-n <value:posint>").action(async ({ session, options }, keyword) => {
    ...
    let searchMsgIds = [];
    if (!options.number) {
      searchMsgIds = await session.send("正在搜索，请稍候...");
    }
    let songs;
    try {
    ...
    if (options.number) {
      ...
      if (config.imageMode) {
        ...
        promptMsgIds = await session.send((0, import_koishi.h)("p", {}, [import_koishi.h.image(image, "image/png"), import_koishi.h.text(prompt)]));
      } else {
        const songListText = songList.map((song, index2) => `${index2 + 1}. ${song}`).join("\n");
        promptMsgIds = await session.send(`${songListText}\n\n${prompt}`);
      }
