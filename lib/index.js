...
var METING_ABNORMAL_RESPONSE_ERROR_NAME = "MetingAbnormalResponseError";
...
var Config = import_koishi.Schema.intersect([
  ...
  import_koishi.Schema.object({
    ...
    enableTextLinkParse: import_koishi.Schema.boolean().default(false).description("是否启用聊天文本音乐链接解析并直接返回音频"),
...
function apply(ctx, config) {
  ...
  let sharedDownloadAgent;
  const getSharedDownloadAgent = /* @__PURE__ */ __name(() => {
    if (!rawProxyUrl) return void 0;
    if (sharedDownloadAgent !== void 0) return sharedDownloadAgent;
    try {
      if (isSocksProxy) {
        const { SocksProxyAgent } = require("socks-proxy-agent");
        sharedDownloadAgent = new SocksProxyAgent(rawProxyUrl);
      } else {
        const { HttpsProxyAgent } = require("https-proxy-agent");
        sharedDownloadAgent = new HttpsProxyAgent(rawProxyUrl);
      }
      return sharedDownloadAgent;
    } catch (error) {
      logger.warn(`[Proxy] 下载代理初始化失败，将回退为直连下载: ${rawProxyUrl}`, error?.message || error);
      sharedDownloadAgent = null;
      return void 0;
    }
  }, "getSharedDownloadAgent");
  function createHttpOptions(extra = {}) {
    const baseOptions = { timeout: 30000, ...extra };
    if (!rawProxyUrl) return baseOptions;
    if (undiciDispatcher) return { ...baseOptions, dispatcher: undiciDispatcher };
    if (proxyConfig) return { ...baseOptions, proxy: proxyConfig };
    return baseOptions;
  }
  ...
  async function retryableGet(url, options, tag) {
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
      try {
        ...
        if (!response) {
            throw new Error(`[${tag}] Empty response`);
        }
        return response;
      } catch (error) {
        lastError = error;
  ...
  async function retryableSend(session, content, tag) {
    if (!session || typeof session.send !== "function") {
      const error = new Error(`[${tag}] session.send 不可用`);
      error.noRetry = true;
      throw error;
    }
    let lastError;
  ...
  __name(normalizeMessageIds, "normalizeMessageIds");
  ...
  const createCachedMediaUrl = /* @__PURE__ */ __name((buf, mimeType = "application/octet-stream", filename = "", options = {}) => {
    ...
    const oldTimer = fileCacheTimers.get(id);
    if (oldTimer) clearTimeout(oldTimer);
    const timer = setTimeout(() => {
      fileCacheTimers.delete(id);
      fileCache.delete(id);
    }, 5 * 60 * 1000);
    timer.unref?.();
    fileCacheTimers.set(id, timer);
  ...
  async function _downloadMediaBuffer(url, tag, options = {}) {
    if (!url || typeof url !== "string") {
      return Promise.reject(new Error(`[${tag}] invalid url`));
    }
    const redirectCount = Number.isFinite(options?.redirectCount) && options.redirectCount >= 0 ? options.redirectCount : 0;
    const maxRedirects = Number.isFinite(options?.maxRedirects) && options.maxRedirects >= 0 ? options.maxRedirects : 5;
    if (redirectCount > maxRedirects) {
      const error = new Error(`[${tag}] too many redirects (${redirectCount})`);
      error.noRetry = true;
      return Promise.reject(error);
    }
    logger.info(`[${tag}] 开始下载: ${url}${rawProxyUrl ? ` (通过代理 ${rawProxyUrl})` : ""}`);
    const agent = getSharedDownloadAgent();
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = mod.get(reqOptions, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          ...
          _downloadMediaBuffer(nextUrl, tag, {
            ...options,
            redirectCount: redirectCount + 1,
            maxRedirects
          }).then((buf) => finish(resolve, buf)).catch((error) => finish(reject, error));
          return;
        }
      ...
      req.setTimeout(30000, () => {
        req.destroy();
        finish(reject, new Error("download socket timeout"));
      });
    });
  }
  ...
  const normalizeExtractedUrl = /* @__PURE__ */ __name((url) => {
    return String(url || "").trim().replace(/[),。！？.!?]+$/g, "");
  }, "normalizeExtractedUrl");
  const extractTextMusicLinks = /* @__PURE__ */ __name((content) => {
    if (!content || typeof content !== 'string') return [];
    const matches = content.match(/https?:\/\/[^\s<>"]+/gi) || [];
    return matches.map((item) => normalizeExtractedUrl(item)).filter(Boolean);
  }, "extractTextMusicLinks");
  const parseNeteaseSongIdFromUrl = /* @__PURE__ */ __name((rawUrl) => {
    try {
      const parsed = new URL(rawUrl);
      const host = parsed.hostname.toLowerCase();
      if (host !== "music.163.com" && host !== "y.music.163.com") return null;
      const parseId = /* @__PURE__ */ __name((value) => {
        const normalized = String(value || "").trim();
        return /^\d+$/.test(normalized) ? normalized : null;
      }, "parseId");
      const pathName = parsed.pathname.replace(/\/+$/, "") || "/";
      if (pathName === "/song") {
        return parseId(parsed.searchParams.get("id"));
      }
      const hash = String(parsed.hash || "").replace(/^#/, "");
      if (!hash) return null;
      const [hashPathRaw, hashQuery = ""] = hash.split("?", 2);
      const hashPath = `/${String(hashPathRaw || "").replace(/^\/+/, "")}`;
      if (hashPath !== "/song") return null;
      const hashParams = new URLSearchParams(hashQuery);
      return parseId(hashParams.get("id"));
    } catch {
      return null;
    }
  }, "parseNeteaseSongIdFromUrl");
  const parseQQSongIdFromUrl = /* @__PURE__ */ __name((rawUrl) => {
    try {
      const parsed = new URL(rawUrl);
      const host = parsed.hostname.toLowerCase();
      if (host !== "y.qq.com") return null;
      const matched = parsed.pathname.match(/^\/n\/ryqq(?:_v2)?\/songDetail\/([^/?#]+)/i);
      if (!matched?.[1]) return null;
      const songId = decodeURIComponent(matched[1]).trim();
      return songId ? songId : null;
    } catch {
      return null;
    }
  }, "parseQQSongIdFromUrl");
  const parseTextMusicLink = /* @__PURE__ */ __name((url) => {
    const neteaseSongId = parseNeteaseSongIdFromUrl(url);
    if (neteaseSongId) {
      return { source: SOURCE_NETEASE, songId: neteaseSongId };
    }
    const qqSongId = parseQQSongIdFromUrl(url);
    if (qqSongId) {
      return { source: DEFAULT_SOURCE, songId: qqSongId };
    }
    return null;
  }, "parseTextMusicLink");
  const parseMetingSongArtists = /* @__PURE__ */ __name((song) => {
    const normalizeArtistName = /* @__PURE__ */ __name((name2) => String(name2 ?? "").trim(), "normalizeArtistName");
    if (Array.isArray(song?.ar)) {
      return song.ar.map((artist) => normalizeArtistName(artist?.name ?? artist)).filter(Boolean).map((name2) => ({ name: name2 }));
    }
    if (Array.isArray(song?.artist)) {
      return song.artist.map((artist) => normalizeArtistName(artist?.name ?? artist)).filter(Boolean).map((name2) => ({ name: name2 }));
    }
    const artistText = String(song?.artist ?? song?.author ?? song?.singer ?? "").trim();
    if (!artistText) {
      return [{ name: "未知歌手" }];
    }
    return artistText.split(/\s*[/|｜、,，]\s*/).map((name2) => normalizeArtistName(name2)).filter(Boolean).map((name2) => ({ name: name2 }));
  }, "parseMetingSongArtists");
  const normalizeMetingSongToSelectedSong = /* @__PURE__ */ __name((song, source, songId) => {
    if (!song || typeof song !== "object") return null;
    const normalizedName = String(song.name ?? song.title ?? "").trim();
    const normalizedArtists = parseMetingSongArtists(song);
    const albumName = String(song.album ?? song.al?.name ?? "").trim() || "未知专辑";
    const picUrl = String(song.pic ?? song.cover ?? song.al?.picUrl ?? "").trim();
    const resolvedSongId = String(song.id ?? song.songid ?? songId ?? "").trim();
    if (!resolvedSongId) return null;
    return {
      id: resolvedSongId,
      name: normalizedName || "未知歌曲",
      ar: normalizedArtists,
      al: {
        name: albumName,
        picUrl
      },
      source: source === SOURCE_NETEASE ? SOURCE_NETEASE : DEFAULT_SOURCE
    };
  }, "normalizeMetingSongToSelectedSong");
  const resolveTextLinkArtist = /* @__PURE__ */ __name((song) => {
    const artistFromArray = Array.isArray(song?.artist) ? song.artist.map((artist) => String(artist?.name ?? artist ?? "").trim()).filter(Boolean).join("/") : "";
    if (artistFromArray) return artistFromArray;
    const artistFromAr = Array.isArray(song?.ar) ? song.ar.map((artist) => String(artist?.name ?? artist ?? "").trim()).filter(Boolean).join("/") : "";
    if (artistFromAr) return artistFromAr;
    const artistText = String(song?.artist ?? song?.author ?? song?.singer ?? "").trim();
    if (!artistText) return "";
    return artistText.split(/\s*[/|｜、,，]\s*/).map((item) => String(item || "").trim()).filter(Boolean).join("/");
  }, "resolveTextLinkArtist");
  const buildTextLinkResponseData = /* @__PURE__ */ __name((song, musicUrl) => {
    if (!song || typeof song !== "object") return null;
    const resolvedMusicUrl = String(musicUrl ?? "").trim();
    if (!resolvedMusicUrl) return null;
    const name2 = String(song.name ?? song.title ?? "").trim();
    const artist = resolveTextLinkArtist(song);
    const coverUrl = String(song.pic ?? song.cover ?? song.al?.picUrl ?? "").trim();
    return {
      name: name2 || "未知歌曲",
      artist: artist || "未知歌手",
  ...
  }, "buildTextLinkResponseData");
  const normalizeNeteaseSongToSelectedSong = /* @__PURE__ */ __name((song, songId) => {
    if (!song || typeof song !== "object") return null;
    const resolvedSongId = String(song.id ?? song.songid ?? songId ?? "").trim();
    if (!resolvedSongId) return null;
    const normalizedName = String(song.name ?? song.title ?? "").trim() || "未知歌曲";
    const artists = Array.isArray(song.ar) ? song.ar.map((artist) => {
      const name2 = String(artist?.name ?? artist ?? "").trim();
      return name2 ? { name: name2 } : null;
    }).filter(Boolean) : parseMetingSongArtists(song);
    const albumName = String(song.al?.name ?? song.album ?? "").trim() || "未知专辑";
    const picUrl = String(song.al?.picUrl ?? song.pic ?? song.cover ?? "").trim();
    return {
      id: resolvedSongId,
      name: normalizedName,
      ar: artists.length ? artists : [{ name: "未知歌手" }],
      al: {
        name: albumName,
        picUrl
      },
  ...
  }, "normalizeNeteaseSongToSelectedSong");
  const createMetingAbnormalResponseError = /* @__PURE__ */ __name((detail) => {
    const error = new Error(`Meting API 返回异常响应: ${detail}`);
    error.name = METING_ABNORMAL_RESPONSE_ERROR_NAME;
    return error;
  }, "createMetingAbnormalResponseError");
  const isMetingAbnormalResponseError = /* @__PURE__ */ __name((error) => {
    return error?.name === METING_ABNORMAL_RESPONSE_ERROR_NAME;
  }, "isMetingAbnormalResponseError");
  async function fetchMetingSongDetailById(songId) {
    const normalizedSongId = String(songId ?? "").trim();
    if (!normalizedSongId) return null;
    const metBase = normalizeApiBase(config.apilink);
    if (!metBase) return null;
    const detailUrl = `${metBase}/api/meting/?type=song&id=${encodeURIComponent(normalizedSongId)}`;
    logInfo(`[Request] Meting Song Detail URL: ${detailUrl}`);
    const response = await retryableGet(detailUrl, createHttpOptions({ headers: createApiHeaders(DEFAULT_SOURCE) }), "Meting Song Detail API");
    const resolved = typeof response === "string" ? (() => {
      try {
        return JSON.parse(response);
      } catch {
        throw createMetingAbnormalResponseError("响应为无法解析的 JSON 字符串");
      }
    })() : response;
    if (resolved == null) return null;
    if (Array.isArray(resolved)) return resolved[0] || null;
    if (typeof resolved !== "object") {
      throw createMetingAbnormalResponseError(`响应终态类型异常: ${typeof resolved}`);
    }
    if ("data" in resolved) {
      if (Array.isArray(resolved.data)) return resolved.data[0] || null;
      if (resolved.data == null) return null;
      if (typeof resolved.data === "object") return resolved.data;
      throw createMetingAbnormalResponseError(`响应 data 字段类型异常: ${typeof resolved.data}`);
    }
    if ("result" in resolved) {
      if (Array.isArray(resolved.result)) return resolved.result[0] || null;
      if (resolved.result == null) return null;
      if (typeof resolved.result === "object") return resolved.result;
      throw createMetingAbnormalResponseError(`响应 result 字段类型异常: ${typeof resolved.result}`);
    }
    return resolved;
  }
  __name(fetchMetingSongDetailById, "fetchMetingSongDetailById");
  async function fetchNeteaseSongDetailById(songId) {
    const normalizedSongId = String(songId ?? "").trim();
    if (!normalizedSongId) return null;
    const neteaseBase = normalizeApiBase(config.neteaseApiLink);
    if (!neteaseBase) return null;
    const detailUrl = `${neteaseBase}/song/detail?ids=${encodeURIComponent(normalizedSongId)}&timestamp=${Date.now()}&randomCNIP=true`;
    logInfo(`[Request] NetEase Song Detail URL: ${detailUrl}`);
    const response = await retryableGet(detailUrl, createHttpOptions({ headers: createApiHeaders(SOURCE_NETEASE) }), "NetEase Song Detail API");
    if (!response || typeof response !== "object") return null;
    if (response.code !== 200) return null;
    const songs = Array.isArray(response.songs) ? response.songs : [];
    if (!songs.length) return null;
    return normalizeNeteaseSongToSelectedSong(songs[0], normalizedSongId);
  }
  __name(fetchNeteaseSongDetailById, "fetchNeteaseSongDetailById");
  async function fetchTextLinkSongMetadata(source, songId) {
    if (source === SOURCE_NETEASE) {
      return fetchNeteaseSongDetailById(songId);
    }
    if (source === DEFAULT_SOURCE) {
      const metingSong = await fetchMetingSongDetailById(songId);
      return normalizeMetingSongToSelectedSong(metingSong, DEFAULT_SOURCE, songId);
    }
    return null;
  }
  __name(fetchTextLinkSongMetadata, "fetchTextLinkSongMetadata");
  async function fetchTextLinkPlayableUrl(source, songId) {
    if (source !== SOURCE_NETEASE && source !== DEFAULT_SOURCE) {
      return "";
    }
    const urlApi = getSongUrlApi(source, songId);
    if (!urlApi) return "";
    logInfo(`[Request] Text Link Song URL: ${urlApi}`);
    const response = await retryableGet(urlApi, createHttpOptions({ headers: createApiHeaders(source) }), "Text Link Song URL API");
    if (!response || response.code !== 200) return "";
    return String(response.data?.[0]?.url ?? "").trim();
  }
  __name(fetchTextLinkPlayableUrl, "fetchTextLinkPlayableUrl");
  let hasLoggedTextLinkMusicUrlRuleDuplicate = false;
  const resolveTextLinkMusicUrlRule = /* @__PURE__ */ __name(() => {
    const fields = Array.isArray(config.return_data_Field) ? config.return_data_Field : [];
    const candidates = fields.filter((field) => field?.data === "musicUrl");
    if (candidates.length > 1 && !hasLoggedTextLinkMusicUrlRuleDuplicate) {
      hasLoggedTextLinkMusicUrlRuleDuplicate = true;
      logger.warn(`文本音乐链接检测到多个 musicUrl 配置，按顺序仅使用首个规则。`);
    }
    return candidates[0] || null;
  }, "resolveTextLinkMusicUrlRule");
  const logAndNext = /* @__PURE__ */ __name((reason, context, next) => {
    if (context !== void 0) {
      logger.warn(`[TextLink] ${reason}`, context);
    } else {
      logger.warn(`[TextLink] ${reason}`);
    }
    return next();
  }, "logAndNext");
  ...
  const extractRequestMetaFromRawInput = /* @__PURE__ */ __name((session, commandName) => {
    ...
    let finalKeyword = remainingText.slice(keywordStartCharIndex).trim();
    if (finalKeyword.startsWith('"') && finalKeyword.endsWith('"')) {
       finalKeyword = finalKeyword.slice(1, -1).trim();
    }
    return {
  ...
  async function fetchSongsByKeyword(session, keyword, requesterTag, source, options = {}) {
    ...
    if (options.showSearchHint) {
      try {
        const result = await retryableSend(session, `${requesterTag}正在搜索，请稍候...`, "搜索提示");
        searchMsgIds = normalizeMessageIds(result);
      } catch (error) {
    ...
    try {
      ...
      if (!response || typeof response !== 'object' || response.code !== 200 || !Array.isArray(response.result?.songs) || !response.result.songs.length) {
  ...
  async function fetchSongsByMixedSources(session, keyword, requesterTag, options = {}) {
    ...
    const mergedSongs = mergeSongsWithMetPriority(metSongs, neteaseSongs);
      if (Array.isArray(mergedSongs) && mergedSongs.length > 0) {
  ...
  async function sendSongSelectionPrompt(session, requesterTag, songs) {
    if (!Array.isArray(songs) || songs.length === 0) return { error: `${requesterTag}没有候选歌曲。` };
    const songList = songs.map((song) => `${song.name || "未知歌曲"} - ${Array.isArray(song.ar) ? song.ar.map((a) => a.name).join("/") : "未知歌手"}`);
    ...
    if (config.imageMode) {
      ...
      try {
        const result = await retryableSend(session, (0, import_koishi.h)("p", {}, [import_koishi.h.image(image, "image/png"), import_koishi.h.text(prompt)]), "歌曲选择列表图片");
        promptMsgIds = normalizeMessageIds(result);
      } catch (error) {
      ...
      try {
        const result = await retryableSend(session, `${requesterTag}候选歌曲如下：\n${songListText}\n\n${prompt}`, "歌曲选择列表文本");
        promptMsgIds = normalizeMessageIds(result);
      } catch (error) {
  ...
  async function beginInteractiveSongRequest(session, request) {
    if (!request || typeof request !== 'object') return `${getRequesterTag(session)}请求无效。`;
    ...
    if (!Array.isArray(songs) || songs.length === 0) {
      return `${requesterTag}未能找到任何相关歌曲。`;
    }
  ...
  async function resolveSelectedSongResponse(session, requesterTag, selectedSong) {
    if (!selectedSong || typeof selectedSong !== 'object') return `${requesterTag}该歌曲信息无效。`;
    ...
    if (!urlApi) {
      ...
      if (!response || typeof response !== 'object' || response.code !== 200 || !Array.isArray(response.data) || !response.data[0]?.url) {
        if (source === SOURCE_NETEASE && response?.code === 200) {
    ...
    const finalData = {
      name: selectedSong.name || "未知歌曲",
      artist: Array.isArray(selectedSong.ar) ? selectedSong.ar.map((a) => a.name).join("/") : "未知歌手",
      album: selectedSong.al?.name || "未知专辑",
  ...
  async function executeSongRequest(session, request) {
    if (!request) return `${getRequesterTag(session)}请求无效。`;
    const { keyword, number, requesterTag } = request;
    if (!keyword) return `${requesterTag}请输入要搜索的歌曲关键词。`;
    const source = request.source === SOURCE_NETEASE ? SOURCE_NETEASE : DEFAULT_SOURCE;
    const searchMode = request.searchMode === SEARCH_MODE_MIXED ? SEARCH_MODE_MIXED : SEARCH_MODE_SINGLE;
    ...
    const selectedSong = songs[number - 1];
    if (!selectedSong) return `${requesterTag}获取所选歌曲信息失败。`;
    return resolveSelectedSongResponse(session, requesterTag, selectedSong);
  }
  ...
  ctx.middleware(async (session, next) => {
    ...
    if (!Number.isInteger(index) || index < 1 || index > removed.songs.length || !removed.songs[index - 1]) {
      try {
  ...
  ctx.on("ready", async () => {
    ...
    if (config.enableMiddleware) {
      ctx.middleware(async (session, next) => {
        ...
        let elements = [];
        try {
          elements = import_koishi.h.parse(session.content);
        } catch (error) {
          logger.warn("音乐卡片消息解析失败，已跳过卡片解析流程:", error?.message || error);
        }
        for (const element of elements) {
          if (!element || typeof element !== 'object' || element.type !== "json" || !element.attrs?.data) continue;
          try {
            ...
            logger.warn("音乐卡片元素解析失败，已跳过该元素:", error?.message || error);
          }
        }
        if (!config.enableTextLinkParse) return next();
        const rawInput = parseSessionInput(session);
        if (!rawInput) return next();
        const links = extractTextMusicLinks(rawInput);
        if (!links.length) return next();
        let parsedTextLink = null;
        for (const link of links) {
          const parsed = parseTextMusicLink(link);
          if (!parsed) continue;
          parsedTextLink = { ...parsed, rawUrl: link };
          break;
        }
        if (!parsedTextLink) return next();
        logInfo(`[Middleware] 检测到文本音乐链接: ${parsedTextLink.rawUrl} | source=${parsedTextLink.source} | songId=${parsedTextLink.songId}`);
        if (parsedTextLink.source !== SOURCE_NETEASE && parsedTextLink.source !== DEFAULT_SOURCE) {
          return logAndNext("source 未识别，跳过文本音乐链接解析。", parsedTextLink, next);
        }
        if (parsedTextLink.source === SOURCE_NETEASE && !String(config.neteaseApiLink || "").trim()) {
          return logAndNext("网易云源未配置，跳过文本音乐链接解析。", parsedTextLink, next);
        }
        if (parsedTextLink.source === DEFAULT_SOURCE && !String(config.apilink || "").trim()) {
          return logAndNext("MET 源未配置，跳过文本音乐链接解析。", parsedTextLink, next);
        }
        let selectedSong = null;
        try {
          selectedSong = await fetchTextLinkSongMetadata(parsedTextLink.source, parsedTextLink.songId);
        } catch (error) {
          if (isMetingAbnormalResponseError(error)) {
            return logAndNext("文本音乐链接详情响应异常。", { parsedTextLink, error: error?.message || error }, next);
          }
          return logAndNext("文本音乐链接详情获取失败。", { parsedTextLink, error: error?.message || error }, next);
        }
        if (!selectedSong) {
          return logAndNext("文本音乐链接解析无结果：未获取到歌曲详情。", parsedTextLink, next);
        }
        let playableUrl = "";
        try {
          playableUrl = await fetchTextLinkPlayableUrl(parsedTextLink.source, selectedSong.id);
        } catch (error) {
          return logAndNext("文本音乐链接播放地址获取失败。", { parsedTextLink, error: error?.message || error }, next);
        }
        if (!playableUrl) {
          return logAndNext("文本音乐链接解析无结果：未获取到可用播放地址。", {
            source: parsedTextLink.source,
            songId: selectedSong.id,
            rawUrl: parsedTextLink.rawUrl
          }, next);
        }
        const responseData = buildTextLinkResponseData(selectedSong, playableUrl);
        if (!responseData) {
          return logAndNext("文本音乐链接响应数据构建失败。", { parsedTextLink, songId: selectedSong.id }, next);
        }
        const musicUrlRule = resolveTextLinkMusicUrlRule();
        if (!musicUrlRule) {
          return logAndNext("未配置 musicUrl 字段规则，跳过文本音乐链接发送。", void 0, next);
        }
        if (musicUrlRule.enable !== true) {
          return logAndNext("musicUrl 字段规则未启用，跳过文本音乐链接发送。", { type: musicUrlRule.type }, next);
        }
        if (musicUrlRule.type === "audio") {
          const sent = await sendTextLinkAudioSilently(session, responseData);
          if (sent) return;
          return logAndNext("文本音乐链接音频发送失败。", { songId: selectedSong.id, source: parsedTextLink.source }, next);
        }
        if (musicUrlRule.type === "file") {
          const sent = await sendTextLinkFileSilently(session, responseData);
          if (sent) return;
          return logAndNext("文本音乐链接文件发送失败。", { songId: selectedSong.id, source: parsedTextLink.source }, next);
        }
        return logAndNext("musicUrl 字段类型无效，仅支持 audio/file。", { type: musicUrlRule.type }, next);
      });
    }
  });
  async function safeRecall(session, messageIds) {
    if (!messageIds || !messageIds.length || typeof session?.bot?.deleteMessage !== 'function' || !session?.channelId) return;
    ...
    <div class="card">
      ...
      if (page) {
        try {
          ...
          logger.warn("关闭 Puppeteer 页面失败:", closeError?.message || closeError);
        }
      }
    }
  }
  __name(generateSongListImage, "generateSongListImage");
  async function sendTextLinkAudioSilently(session, data) {
    const musicUrl = String(data?.musicUrl ?? "").trim();
    if (!musicUrl) {
      logger.warn("文本音乐链接发送跳过：musicUrl 为空。", { name: data?.name, artist: data?.artist });
      return false;
    }
    if (!rawProxyUrl) {
      logger.warn("文本音乐链接发送跳过：未配置 proxyUrl。", { musicUrl });
      return false;
    }
    try {
      const actualMaxBytes = ctx.server ? 200 * 1024 * 1024 : inlineMediaMaxBytes;
      const buf = await downloadMediaBuffer(musicUrl, "TextLinkAudio", { maxBytes: actualMaxBytes });
      if (!buf || !buf.length) {
         logger.warn("文本音乐链接音频下载为空。", { musicUrl });
         return false;
      }
      let mimeType = "audio/mpeg";
      if (musicUrl.includes(".flac")) mimeType = "audio/flac";
      else if (musicUrl.includes(".ogg")) mimeType = "audio/ogg";
      else if (musicUrl.includes(".wav")) mimeType = "audio/wav";
      else if (musicUrl.includes(".m4a") || musicUrl.includes(".aac")) mimeType = "audio/mp4";
      const safeName = String(data?.name ?? "").replace(/[\\/:*?"<>|]/g, "_");
      const safeArtist = String(data?.artist ?? "").replace(/[\\/:*?"<>|]/g, "_");
      const audioExt = musicUrl.includes(".flac") ? "flac" : musicUrl.includes(".ogg") ? "ogg" : musicUrl.includes(".wav") ? "wav" : musicUrl.includes(".m4a") || musicUrl.includes(".aac") ? "m4a" : "mp3";
      ...
      if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
        const remoteUrl = selectRemoteUploadUrl(musicUrl, buf, mimeType, audioFilename, "TextLinkAudio");
        if (!remoteUrl) {
          logger.warn("文本音乐链接音频发送失败：大文件远程上传地址不可用。", {
            size: buf.length,
            threshold: onebotDirectUploadThresholdBytes,
            musicUrl
          });
          return false;
        }
        await retryableSend(session, (0, import_koishi.h)("file", { src: remoteUrl, title: audioFilename, name: audioFilename }), "文本链接音频消息");
        return true;
      }
      if (oneBotLike) {
        await retryableSend(session, import_koishi.h.audio(`base64://${buf.toString("base64")}`, mimeType), "文本链接音频消息");
        return true;
      }
      let fileUrl;
      if (ctx.server && buf.length > inlineMediaMaxBytes) {
        fileUrl = createCachedMediaUrl(buf, mimeType, audioFilename);
        if (!fileUrl) {
          logger.warn("文本音乐链接音频发送失败：超限且未生成可访问缓存地址。", {
            size: buf.length,
            limit: inlineMediaMaxBytes,
            musicUrl
          });
          return false;
        }
      ...
      await retryableSend(session, import_koishi.h.audio(fileUrl, mimeType), "文本链接音频消息");
      return true;
    } catch (error) {
      logger.warn("文本音乐链接音频发送失败，已静默跳过：", error?.message || error);
      return false;
    }
  }
  __name(sendTextLinkAudioSilently, "sendTextLinkAudioSilently");
  async function sendTextLinkFileSilently(session, data) {
    const musicUrl = String(data?.musicUrl ?? "").trim();
    if (!musicUrl) {
      logger.warn("文本音乐链接文件发送跳过：musicUrl 为空。", { name: data?.name, artist: data?.artist });
      return false;
    }
    if (!rawProxyUrl) {
      logger.warn("文本音乐链接文件发送跳过：未配置 proxyUrl。", { musicUrl });
      return false;
    }
    let ext = "mp3";
    try {
      const parsedPath = new URL(musicUrl).pathname;
    ...
    try {
      const actualMaxBytes = ctx.server ? 200 * 1024 * 1024 : inlineMediaMaxBytes;
      const buf = await downloadMediaBuffer(musicUrl, "TextLinkFile", { maxBytes: actualMaxBytes });
      if (!buf || !buf.length) {
         logger.warn("文本音乐链接文件下载为空。", { musicUrl });
         return false;
      }
      const oneBotLike = isOneBotLikePlatform(session.platform);
      if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
        const remoteUrl = selectRemoteUploadUrl(musicUrl, buf, "application/octet-stream", filename, "TextLinkFile");
        if (!remoteUrl) {
          logger.warn("文本音乐链接文件发送失败：大文件远程上传地址不可用。", {
            size: buf.length,
            threshold: onebotDirectUploadThresholdBytes,
            musicUrl
          });
        ...
        await retryableSend(session, (0, import_koishi.h)("file", { src: remoteUrl, title: filename, name: filename }), "文本链接文件消息");
        return true;
      }
      let fileUrl;
      if (oneBotLike) {
        ...
        if (!fileUrl) {
          logger.warn("文本音乐链接文件发送失败：超限且未生成可访问缓存地址。", {
            size: buf.length,
            limit: inlineMediaMaxBytes,
            musicUrl
          });
      ...
      await retryableSend(session, (0, import_koishi.h)("file", { src: fileUrl, title: filename, name: filename }), "文本链接文件消息");
      return true;
    } catch (error) {
      logger.warn("文本音乐链接文件发送失败，已静默跳过：", error?.message || error);
      return false;
    }
  }
  __name(sendTextLinkFileSilently, "sendTextLinkFileSilently");
...
`));
    ...
    if (oneBotLike) {
      const nickname = session.author?.nickname || session.username || session.bot?.nickname || session.bot?.username || "未知用户";
      const userId = session.author?.userId || session.bot?.selfId || "unknown";
      try {
        if (config.isFigure) {
  ...
  ctx.on("dispose", async () => {
    for (const timer of fileCacheTimers.values()) {
      clearTimeout(timer);
    }
    fileCacheTimers.clear();
    fileCache.clear();
    for (const pendingState of pendingSongSelections.values()) {
      if (pendingState?.timeout) {
        clearTimeout(pendingState.timeout);
      }
    }
    pendingSongSelections.clear();
    if (typeof undiciDispatcher?.close === "function") {
      try {
        await undiciDispatcher.close();
      } catch (error) {
        logger.warn("关闭 undici 代理调度器失败:", error?.message || error);
      }
    }
    if (typeof sharedDownloadAgent?.destroy === "function") {
      try {
        sharedDownloadAgent.destroy();
      } catch (error) {
        logger.warn("关闭下载代理失败:", error?.message || error);
      }
    }
  });
}
