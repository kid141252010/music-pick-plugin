...
function apply(ctx, config) {
  ...
  async function fetchSongsByKeyword(session, keyword, requesterTag, options = {}) {
    if (!config.apilink) {
  ...
  async function beginInteractiveSongRequest(session, request) {
    ...
    const searchResult = await fetchSongsByKeyword(session, keyword, requesterTag, { showSearchHint: true });
  ...
  async function executeSongRequest(session, request) {
    ...
    if (!number) {
      return beginInteractiveSongRequest(session, { ...request, keyword, requesterTag });
    }
    const searchResult = await fetchSongsByKeyword(session, keyword, requesterTag);
    if (searchResult.error) return searchResult.error;
    const songs = searchResult.songs;
