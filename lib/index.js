...
function apply(ctx, config) {
  ...
  function createMediaDownloadOptions(withReferer = true) {
  ...
  async function downloadMediaBuffer(url, tag) {
    try {
      const arrayBuffer = await retryableGet(url, createMediaDownloadOptions(true), `${tag} Download`);
      return Buffer.from(arrayBuffer);
    } catch (error) {
      const status = error?.response?.status;
      if (status === 401 || status === 403) {
        logger.warn(`[${tag}] 带 Referer 下载被拒绝(status=${status})，尝试无 Referer 重试。`);
        const arrayBuffer = await retryableGet(url, createMediaDownloadOptions(false), `${tag} Download (No Referer)`);
        return Buffer.from(arrayBuffer);
      }
      throw error;
    }
...
`));
        ...
        case "audio": {
          ...
          try {
            ...
            logger.warn(`音频下载失败(status=${status ?? "unknown"})，不发送下载地址:`, e?.response?.data || e?.message || e);
