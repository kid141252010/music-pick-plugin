...
function apply(ctx, config) {
  ...
  ctx.command(`${config.commandName} <keyword:text>`, "搜索和点播音乐。").option("number", "-n <value:posint>").action(async ({ session, options }, keyword) => {
    ...
    if (!options.number) {
      try {
        searchMsgIds = normalizeMessageIds(await retryableSend(session, "正在搜索，请稍候...", "搜索提示"));
      } catch (error) {
    ...
    try {
      ...
      if (response.code !== 200 || !response.result?.songs?.length) {
        if (searchMsgIds.length) safeRecall(session, searchMsgIds);
    ...
    if (options.number) {
      ...
      if (config.imageMode) {
        ...
        try {
          ...
          return "发送歌曲选择列表失败，请稍后重试。";
        }
      } else {
        const songListText = songList.map((song, index2) => `${index2 + 1}. ${song}`).join("\n");
        try {
          promptMsgIds = normalizeMessageIds(await retryableSend(session, `${songListText}\n\n${prompt}`, "歌曲选择列表文本"));
        } catch (error) {
      ...
      if (promptMsgIds.length) safeRecall(session, promptMsgIds);
