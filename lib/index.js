...
function apply(ctx, config) {
  ...
  const resolveServerDownloadBaseUrl = /* @__PURE__ */ __name((requirePublic = false) => {
    ...
    if (isHttpUrl(configuredSelfUrl)) {
      const parsed = new URL(configuredSelfUrl);
      if (!requirePublic || !isLoopbackHostname(parsed.hostname)) {
        return configuredSelfUrl.replace(/\/+$/, "");
      }
    }
    if (requirePublic) return "";
    const serverSelfUrl = ctx.server?.selfUrl;
    if (isHttpUrl(serverSelfUrl)) {
      return serverSelfUrl.replace(/\/+$/, "");
    }
    ...
    return `http://${resolvedHost}:${koishiPort}`;
  ...
  const createCachedMediaUrl = /* @__PURE__ */ __name((buf, mimeType = "application/octet-stream") => {
    if (!ctx.server) return "";
    const baseUrl = resolveServerDownloadBaseUrl(true);
    if (!baseUrl) return "";
        ...
        case "audio": {
          ...
          try {
            ...
            if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
              let remoteUrl = isPublicHttpUrl(value) ? value : "";
              if (!remoteUrl) {
                remoteUrl = createCachedMediaUrl(buf, mimeType);
              }
              if (!remoteUrl) {
              ...
              const baseUrl = resolveServerDownloadBaseUrl(oneBotLike);
              if (!baseUrl) {
        ...
        case "file": {
          ...
          try {
            ...
            if (oneBotLike && buf.length > onebotDirectUploadThresholdBytes) {
              let remoteUrl = isPublicHttpUrl(value) ? value : "";
              if (!remoteUrl) {
                remoteUrl = createCachedMediaUrl(buf, "application/octet-stream");
              }
              if (!remoteUrl) {
              ...
              const id = Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
              fileCache.set(id, { buf, mimeType: "application/octet-stream" });
              setTimeout(() => fileCache.delete(id), 5 * 60 * 1000);
              fileUrl = `${baseUrl}/music-pick/${id}`;
            } else {
